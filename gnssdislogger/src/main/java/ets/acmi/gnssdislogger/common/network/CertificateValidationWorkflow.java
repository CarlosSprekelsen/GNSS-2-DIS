...
class CertificateValidationWorkflow implements Runnable {

    private static final Logger LOG = Logs.of(CertificateValidationWorkflow.class);
    private final Handler postValidationHandler;
        ...
        if (!isValid) {
            ...
            if (cve != null) {
                ...
                try {
                    ...
                    if (sans.length() > 0) {
                        sb.append(String.format(msgformat, "Subject Alternative Names", sans));
                    }
                    ...
                    new MaterialDialog.Builder(context)
                            ...
                            .content(HtmlCompat.fromHtml(sb.toString(), HtmlCompat.FROM_HTML_MODE_LEGACY))
                            ...
                            .onPositive((dialog, which) -> {
                                try {
                                    ...
                                    LOG.error("Could not add to the keystore", e12);
                                }
    ...
    public void run() {
        try {
            ...
            try {
                ...
                postValidationHandler.post(() -> onWorkflowFinished(context, null, true));
                ...
                if (serverType == ServerType.HTTPS) {
                    postValidationHandler.post(() -> onWorkflowFinished(context, e, false));
                    return;
                }
                ...
                postValidationHandler.post(() -> onWorkflowFinished(context, null, false));
            }
